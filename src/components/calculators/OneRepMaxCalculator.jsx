...
// Interactive one-rep-max calculator widget, extracted from
// /calculators/one-rep-max/page.jsx for reuse by landing pages. Owns its own
// Suspense boundary (uses useSearchParams). `prefill` optionally overrides
// initial inputs ({ weight, reps, unit, exercise, experience }) — landing pages
// use it to preselect an exercise.
function OneRepMaxCalculatorInner({ prefill = {} }) {
  ...
  const [exercise, setExercise] = useState(prefill.exercise ?? "bench-press");
  const [experience, setExperience] = useState(prefill.experience ?? "intermediate");
...
export default function OneRepMaxCalculator({ prefill }) {
  return (
    <Suspense fallback={<div className="max-w-4xl mx-auto px-4 py-12">Loading calculator…</div>}>
      <OneRepMaxCalculatorInner prefill={prefill} />
    </Suspense>
  );
}
