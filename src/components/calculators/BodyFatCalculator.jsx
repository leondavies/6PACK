...
// Interactive body-fat (Navy method) calculator widget, extracted from
// /calculators/body-fat/page.jsx for reuse by landing pages. Owns its own
// Suspense boundary (uses useSearchParams). `prefill` optionally overrides
// initial inputs ({ height, neck, waist, hip, gender, fitnessLevel, unit }).
function BodyFatCalculatorInner({ prefill = {} }) {
  const searchParams = useSearchParams();
  const [height, setHeight] = useState(prefill.height ?? "");
  const [neck, setNeck] = useState(prefill.neck ?? "");
  const [waist, setWaist] = useState(prefill.waist ?? "");
  const [hip, setHip] = useState(prefill.hip ?? "");
  const [gender, setGender] = useState(prefill.gender ?? "male");
  const [fitnessLevel, setFitnessLevel] = useState(prefill.fitnessLevel ?? "intermediate");
                ...
                <div className="space-y-4">
                  <div className="bg-primary-50 border border-primary-200 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-primary-900">Body Fat Percentage</h4>
                        <p className="text-sm text-primary-700">{category}</p>
                      </div>
                      <div className="text-3xl font-bold text-primary-600">{bodyFat}%</div>
                    </div>
                  </div>
                </div>
              </div>
...
export default function BodyFatCalculator({ prefill }) {
  return (
    <Suspense fallback={<div className="max-w-4xl mx-auto px-4 py-12">Loading calculator…</div>}>
      <BodyFatCalculatorInner prefill={prefill} />
    </Suspense>
  );
}
